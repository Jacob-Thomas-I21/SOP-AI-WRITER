...
from app.models.audit import (
    AuditLog, AuditLogCreate, AuditLogResponse, AuditSummary,
    ComplianceReport, DataIntegrityCheck, AuditAction, AuditSeverity
)
...
class AuditService:
    ...
    async def get_audit_summary(
        ...
        try:
            ...
            events_by_severity = {str(severity): count for severity, count in severity_results}
    ...
    async def generate_compliance_report(
        ...
        try:
            ...
            fda_compliance = self._assess_fda_21_cfr_part_11(session, start_date, end_date)
            ...
            missing_signatures = self._find_missing_signatures(session, start_date, end_date)
            
            # Find incomplete records
            incomplete_records = self._find_incomplete_records(session, start_date, end_date)
            
            # Identify high-risk activities
            high_risk_activities = self._identify_high_risk_activities(session, start_date, end_date)
            
            # Check for regulatory violations
            regulatory_violations = self._check_regulatory_violations(session, start_date, end_date)
    ...
    async def perform_data_integrity_check(
        ...
        try:
            ...
            check_result.is_valid = (
                check_result.checksum_valid and
                check_result.digital_signature_valid and
                check_result.modification_history_complete and
                check_result.access_controls_verified
            )
