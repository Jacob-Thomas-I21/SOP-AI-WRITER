...
class OllamaClient:
    ...
    async def pull_model(self) -> bool:
        ...
        try:
            ...
            await self._make_request("POST", "/api/pull", data=data, timeout=600)  # 10 minutes
    ...
    async def generate_sop(
        ...
        try:
            ...
            response = await self._make_request("POST", "/api/generate", data=request_data)
    ...
    async def _parse_generated_sop(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            section_patterns = [
                "1. Title Page", "Title Page", "TITLE PAGE",
                "2. Purpose", "Purpose", "PURPOSE", 
                "3. Scope", "Scope", "SCOPE",
                "4. Definitions", "Definitions", "DEFINITIONS", "Abbreviations",
                "5. Responsibilities", "Responsibilities", "RESPONSIBILITIES",
                "6. Materials", "Materials", "MATERIALS", "Equipment",
                "7. Procedure", "Procedure", "PROCEDURE", "Method",
                "8. Acceptance Criteria", "Acceptance Criteria", "ACCEPTANCE CRITERIA",
                "9. Documentation", "Documentation", "DOCUMENTATION", "Records",
                "10. References", "References", "REFERENCES",
                "11. Revision History", "Revision History", "REVISION HISTORY"
            ]
            ...
            for line in lines:
                ...
                for pattern in section_patterns:
                    if line.startswith(pattern) or line == pattern:
                        ...
                        # Start new section
                        current_section = pattern.split('. ')[-1].lower().replace(' ', '_')
                        current_content = []
            ...
            for section in required_sections:
                if section not in sections:
                    sections[section] = f"[Generated content for {section.replace('_', ' ').title()} section - please review and complete]"
    ...
    async def _calculate_quality_score(
        ...
        try:
            ...
            required_sections = ["purpose", "scope", "procedure", "responsibilities", "materials", "documentation"]
            completed_sections = sum(1 for section in required_sections if section in sections and len(sections[section]) > 50)
            ...
            pharma_terms = [
                "gmp", "fda", "validation", "qualification", "compliance", 
                "batch", "lot", "deviation", "capa", "quality control",
                "quality assurance", "manufacturing", "equipment", "cleaning",
                "sterile", "aseptic", "contamination", "specification"
            ]
            ...
            logger.info(f"Quality score calculated: {score}/100 (sections: {section_score}, length: {length_score}, terminology: {terminology_score}, structure: {structure_score})")
    ...
    async def validate_pharmaceutical_terminology(self, content: str) -> List[PharmaceuticalTerminology]:
        ...
        try:
            ...
            for term, definition in standard_terms.items():
                if term in content_lower:
                    terms_found.append(PharmaceuticalTerminology(
                        term=term.upper(),
    ...
    async def get_model_info(self) -> Dict[str, Any]:
        ...
        try:
            response = await self._make_request("POST", "/api/show", data={"name": self.model})
            return {
