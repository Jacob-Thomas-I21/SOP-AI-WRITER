...
class PDFGenerator:
    ...
    async def generate_pdf(
        ...
        try:
            ...
            if sop.sop_content and sop.sop_content.get('structured_sections'):
                self._add_content_sections(story, sop.sop_content['structured_sections'])
            ...
            logger.info(f"PDF generated successfully: {filename} ({pdf_size} bytes) in {generation_time:.2f}s")
    ...
    def _add_document_header(self, story: List, sop: SOP, generation_time: datetime):
        ...
        doc_info_data = [
            ...
            ['Department', sop.department.title() if sop.department else 'N/A', 
             'Priority', sop.priority.title() if sop.priority else 'Medium'],
            ['Created By', sop.created_by, 'Generated Date', generation_time.strftime("%B %d, %Y")],
            ['Status', sop.status.title() if hasattr(sop.status, 'title') else str(sop.status), 
             'FDA Score', f"{sop.fda_compliance_score}%" if sop.fda_compliance_score else 'Pending']
        ]
        ...
        doc_info_table = Table(doc_info_data, colWidths=[1.5*inch, 2*inch, 1.5*inch, 2*inch])
        ...
        story.append(Spacer(1, 20))
        story.append(HRFlowable(width="100%", thickness=3, color=colors.HexColor('#0066cc')))
        story.append(Spacer(1, 20))
                ...
                if section_key.lower() == 'procedure':
                    ...
                    for i, step in enumerate(steps, 1):
                        if step.strip():
                            ...
                            story.append(Paragraph(step_text, self.styles['ProcedureStep']))
                else:
                    # Regular content
    ...
    def _add_gmp_compliance_section(self, story: List, gmp_indicators: Dict[str, bool]):
        ...
        story.append(Paragraph("GMP Compliance Indicators", self.styles['SectionHeader']))
        ...
        for indicator, status in gmp_indicators.items():
            ...
            status_color = colors.HexColor('#28a745') if status else colors.HexColor('#dc3545')
    ...
    def _add_regulatory_section(self, story: List, sop: SOP):
        ...
        if sop.guideline_refs:
            story.append(Paragraph("<b>Applicable Regulations:</b>", self.styles['Normal']))
    ...
    def _add_signature_section(self, story: List):
        ...
        story.append(Paragraph("Authorization Signatures", self.styles['SectionHeader']))
        ...
        sig_table = Table(sig_data, colWidths=[3*inch, 3*inch], rowHeights=[0.3*inch, 0.5*inch, 0.3*inch, 0.3*inch, 0.5*inch])
    ...
    def _add_document_control_section(self, story: List, sop: SOP, generation_time: datetime):
        ...
        rev_table = Table(rev_data, colWidths=[0.8*inch, 1.2*inch, 2.5*inch, 1.2*inch, 1.3*inch])
    ...
    async def generate_batch_pdfs(self, sops: List[SOP]) -> Dict[str, Any]:
        ...
        logger.info(f"Batch PDF generation completed: {len(results['successful'])}/{len(sops)} successful")
        ...
        templates = [
            {
                ...
                "features": ["Regulatory headers", "Signature blocks", "GMP compliance indicators", "Audit trail ready"],
                "pdf_library": "ReportLab"
            }
        ]
    ...
    def cleanup_old_pdfs(self, days_old: int = 30) -> int:
        ...
        try:
            ...
            logger.info(f"Cleaned up {deleted_count} PDF files older than {days_old} days")
    ...
    def get_storage_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "average_file_size_bytes": round(total_size / total_files) if total_files > 0 else 0,
                "storage_directory": str(self.output_dir),
                "oldest_file": min(pdf_files, key=lambda f: f.stat().st_mtime).name if pdf_files else None,
                "newest_file": max(pdf_files, key=lambda f: f.stat().st_mtime).name if pdf_files else None,
                "pdf_library": "ReportLab"
            }
