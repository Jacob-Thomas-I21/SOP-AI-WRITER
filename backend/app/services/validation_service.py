...
class PharmaceuticalValidationService:
    ...
    async def validate_sop_content(
        self, sop_content: Dict[str, Any], regulatory_frameworks: List[str]
    ) -> SOPValidationResult:
        ...
        try:
            ...
            validation_result.is_valid = (
                validation_result.compliance_score >= 70.0
            ...
            logger.info(
                f"SOP validation completed - Score: {validation_result.compliance_score:.2f}%"
            )
            ...
            return SOPValidationResult(
                ...
                gmp_compliance_indicators={},
                recommendations=[
                    "Contact system administrator - validation service error"
                ],
            )
    ...
    async def _validate_sections(
        ...
        for framework in frameworks:
            if framework.lower() in self.regulatory_requirements:
                req_sections = self.regulatory_requirements[framework.lower()][
                    "required_sections"
                ]
        ...
        for section in required_sections:
            ...
            for variation in section_variations:
                for key, content in sections.items():
                    if (
                        variation.lower() in key.lower()
                        or key.lower() in variation.lower()
                    ):
                        section_found = True
    ...
    async def _validate_regulatory_compliance(
        self, content: str, frameworks: List[str], result: SOPValidationResult
    ) -> float:
    ...
    async def _validate_terminology(
        self, content: str, result: SOPValidationResult
    ) -> float:
    ...
    async def _validate_gmp_compliance(
        self, content: str, sections: Dict[str, str], result: SOPValidationResult
    ) -> float:
        """Validate GMP compliance indicators."""
    ...
    async def _generate_recommendations(self, result: SOPValidationResult):
        ...
        if result.compliance_score < 50:
            result.recommendations.insert(
                0, "Major revision required - compliance score below 50%"
            )
        elif result.compliance_score < 70:
            result.recommendations.insert(
                0, "Moderate revision recommended - compliance score below 70%"
            )
        elif result.compliance_score < 85:
            result.recommendations.insert(
                0, "Minor improvements suggested for optimal compliance"
            )
        ...
        non_compliant_indicators = [
            indicator
            for indicator, status in result.gmp_compliance_indicators.items()
            if not status
        ]
    ...
    async def validate_batch_sops(
        self, sops_content: List[Dict[str, Any]], regulatory_frameworks: List[str]
        ...
        for sop_content in sops_content:
            try:
                result = await self.validate_sop_content(
                    sop_content, regulatory_frameworks
                )
