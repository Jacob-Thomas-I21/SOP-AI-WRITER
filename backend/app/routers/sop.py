...
from app.models.sop import (
    SOP, SOPCreate, SOPUpdate, SOPResponse, SOPSearchFilters, 
    SOPStatus, SOPValidationResult, PharmaceuticalTerminology
)
...
async def create_sop_generation_job(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create SOP generation job: {str(e)}")


async def generate_sop_background(job_id: str, sop_data: dict, template_content: str, guideline_content: str):
    ...
    try:
        ...
        if generation_result['status'] == 'completed':
            ...
            sop.validation_errors = validation_result.validation_errors if not validation_result.is_valid else []
...
async def search_sops(
    ...
    try:
        ...
        if filters.min_compliance_score is not None:
            query = query.where(SOP.fda_compliance_score >= filters.min_compliance_score)
...
async def validate_pharmaceutical_terminology(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Terminology validation failed: {str(e)}")
