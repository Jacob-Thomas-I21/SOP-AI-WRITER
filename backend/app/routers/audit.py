...
async def get_user_audit_activity(
    ...
    try:
        ...
        for log in user_logs:
            ...
            action_type = log.action.value if hasattr(log.action, 'value') else str(log.action)
            activity_summary["actions_by_type"][action_type] = activity_summary["actions_by_type"].get(action_type, 0) + 1
            
            # Track resources
            activity_summary["resources_accessed"].add(f"{log.resource_type}:{log.resource_id}")
        ...
        activity_summary["unique_resources_accessed"] = len(activity_summary["resources_accessed"])
...
async def get_critical_system_events(
    ...
    try:
        ...
        for event in critical_events:
            ...
            events_summary["summary_by_type"][event_type] = events_summary["summary_by_type"].get(event_type, 0) + 1
