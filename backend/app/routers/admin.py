    ...
    try:
        ...
        stats = {
            "users": {
                ...
                "last_week": session.exec(select(func.count(User.id)).where(User.created_at >= last_week)).one(),
                "last_month": session.exec(select(func.count(User.id)).where(User.created_at >= last_month)).one()
            },
            "sops": {
                "total": session.exec(select(func.count(SOP.id))).one(),
                "completed": session.exec(select(func.count(SOP.id)).where(SOP.status == "completed")).one(),
                "pending": session.exec(select(func.count(SOP.id)).where(SOP.status == "pending")).one(),
                "failed": session.exec(select(func.count(SOP.id)).where(SOP.status == "failed")).one(),
                "last_week": session.exec(select(func.count(SOP.id)).where(SOP.created_at >= last_week)).one(),
                "last_month": session.exec(select(func.count(SOP.id)).where(SOP.created_at >= last_month)).one()
            },
            "templates": {
                "total": session.exec(select(func.count(Template.id))).one(),
                "active": session.exec(select(func.count(Template.id)).where(Template.is_active == True)).one()
            },
            "audit_logs": {
                "total": session.exec(select(func.count(AuditLog.id))).one(),
                "last_week": session.exec(select(func.count(AuditLog.id)).where(AuditLog.timestamp >= last_week)).one(),
                "critical_events": session.exec(select(func.count(AuditLog.id)).where(AuditLog.severity == "critical")).one()
            },
            "system_info": {
