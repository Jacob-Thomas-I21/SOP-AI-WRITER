...
class TemplateBase(SQLModel):
    ...
    prerequisite_templates: List[str] = Field(
        default_factory=list, sa_column=Column(JSON)
    )
...
class Template(TemplateBase, table=True):
    ...
    global_validation_rules: List[ValidationRule] = Field(
        default_factory=list, sa_column=Column(JSON)
    )

    # Pharmaceutical-specific metadata
    gmp_compliance_requirements: Dict[str, Any] = Field(
        default_factory=dict, sa_column=Column(JSON)
    )
    equipment_categories: List[str] = Field(
        default_factory=list, sa_column=Column(JSON)
    )
    chemical_safety_requirements: Dict[str, Any] = Field(
        default_factory=dict, sa_column=Column(JSON)
    )
...
class TemplateCreate(TemplateBase):
    ...
    global_validation_rules: Optional[List[ValidationRule]] = Field(
...
class TemplateSearchFilters(BaseModel):
    ...
    sort_by: Optional[str] = Field(
        default="usage_count",
        regex=r"^(created_at|updated_at|name|usage_count|quality_score|regulatory_compliance_score)$",
    )
