...
class RegulatoryHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord):
        ...
        if getattr(record, "regulatory_event", False) or getattr(
            record, "gmp_relevant", False
        ):
            try:
...
def get_pharmaceutical_logger(
    ...
    extra_context = {"pharmaceutical_context": True, "gmp_system": True}
    ...
    def archive_old_logs(self, retention_days: int = 2555):  # 7 years default
        ...
        for log_file in self.log_dir.glob("*.log*"):
            if log_file.stat().st_mtime < cutoff_date.timestamp():
                ...
                archive_name = (
                    f"{log_file.stem}_{datetime.now().strftime('%Y%m%d')}.tar.gz"
                )
